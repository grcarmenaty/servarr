...
| `docs/10-cloud-stack.md` | Load-balanced 2×Nextcloud + Firefly III, shared media folder |
...
10. Deploy the cloud tier — two load-balanced Nextcloud instances +
    Firefly III with the shared media folder (`docs/10-cloud-stack.md`
    / `scripts/13` + `cloud/`).
11. `bash scripts/20-enable-ha.sh` — HA-enroll everything, with
    anti-affinity for the Nextcloud and DNS pairs (`docs/06`).
12. Verify with `scripts/99-health-check.sh` at any point.
