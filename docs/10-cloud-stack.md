...
**First-boot note**: cloud-data runs ~27 containers (this compose +
Taiga's 7) and is sized at 16 GB for it (docs/06). On the very first
`docker compose up -d` everything initializes, migrates databases, and
builds search indexes at once — expect a few minutes of high load and
let it settle before judging. If a container is stuck restarting, bring
the stack up in waves (`docker compose up -d db redis` first, then the
rest) rather than assuming a failure. Steady-state RAM is far below the
first-boot spike.
